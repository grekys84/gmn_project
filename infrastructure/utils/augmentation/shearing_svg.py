...
DEFAULT_SAVE_DIR = settings.data.augmentation_shear
shearing_width_range = (
    settings.augmentation_shear.shearing_width_range
)  # (min, max)
shearing_length_range = (
    settings.augmentation_shear.shearing_length_range
)  # (min, max)
shearing_per_file = settings.augmentation_shear.shearing_per_file
min_edge_length = settings.augmentation_shear.min_edge_length

log_file = settings.logging.log_dir / "augmentation_shear.log"
...
def create_shearing_on_edge(
    ...
    """Создаёт shearing (тонкую линию), гарантированно наружу."""
    x1, y1 = start_point
    x2, y2 = end_point
    ...
    # точки на конце линии
    tip1_x, tip1_y = base1_x + nx * length, base1_y + ny * length
    tip2_x, tip2_y = base2_x + nx * length, base2_y + ny * length
...
def process_svg_file(input_path, output_path, log_file=None):
    """Обрабатывает рёбра: добавляет shearing наружу."""
    try:
        ...
        total_shearings_added = 0
        num_shearings = min(shearing_per_file, len(edges))
        selected_edges = random.sample(edges, num_shearings)

        for edge in selected_edges:
            position_t = random.uniform(0.2, 0.8)
            width = random.uniform(*shearing_width_range)
            length = random.uniform(*shearing_length_range)

            shearing_points = create_shearing_on_edge(
                edge["start_point"],
                edge["end_point"],
                width,
                length,
                ...
                path_str = (
                    ...
                    + " ".join(
                        [
                            ...
                            for i, (x, y) in enumerate(shearing_points)
                        ]
                    )
                    + " Z"
                )
                new_shearing = ET.SubElement(root, "path")
                new_shearing.set("d", path_str)
                cls = edge["elem"].get("class")
                if cls:
                    new_shearing.set("class", cls)
                total_shearings_added += 1
        ...
        return save_tree(
            tree,
            output_path,
            msg=f"{input_path.name}: добавлено {total_shearings_added}"
            f" shearing-линий",
            log_file=log_file,
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Добавление shearing-линий наружу вдоль рёбер SVG"
    )
    parser.add_argument(
        "input",
        nargs="?",
